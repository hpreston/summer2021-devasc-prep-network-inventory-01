...
def get_device_inventory(device, show_version, show_inventory): 
    ...
    if device.os in ["ios", "iosxe"]: 
        ...
        software_version_regex = "Software\sVersion\s([^\n ]*)"
        software_version = re.search(software_version_regex, show_version[device.name]["output"]).group(1)

        # match = software_version_regex.match(asa_show_version["output"])

        # x = re.search(r"^.*Software\sVersion\s([^\n ]*)", '\r\nCisco Adaptive Security Appliance Software Version 9.12(2) \r\n')
        # x = re.search(r"^.*Software\sVersion\s", '\r\nCisco Adaptive Security Appliance Software Version 9.12(2) \r\n')

        # x = re.search("Software\sVersion\s([^\n ]*)", "\r\nCisco Adaptive Security Appliance Software Version 9.12(2) \r\n")
        # x = re.search("Software\sVersion\s([^\n ]*)", asa_show_version["output"])
